    ...
    public partial class HomeController : Controller
    {
        ...
        public ActionResult TracNghiem(IFormCollection f, IFormFile txtFile)
        {
            try
            {
                ...
                for(var i = 0; i < noidungSD.Length; i++)
                {
                    var htmlSanitizerAccept = noidungSD[i].Replace("\r", "").Split("\n");
            ...
            try
            {
                ...
                if (f["File_ND"] == "")
                {
                    ...
                    else
                    {
                        ...
                        if (hourDL.Contains("-"))
                        {
                            xong.AddHours(-1 * int.Parse(hourDL.Replace("-", "")));
                        }
                        else
                        {
                            xong.AddHours(int.Parse(hourDL));
                        }

                        if (minDL.Contains("-"))
                        {
                            xong.AddMinutes(-1 * int.Parse(minDL.Replace("-", "")));
                        }
                        else
                        {
                            xong.AddHours(int.Parse(minDL));
                        }

                        if (secDL.Contains("-"))
                        {
                            xong.AddSeconds(-1 * int.Parse(secDL.Replace("-", "")));
                        }
                        else
                        {
                            xong.AddSeconds(int.Parse(secDL));
                        }
                ...
                else
                {
                    ...
                    var noidung = FileExtension.ReadFile(pathXY);
                    ...
                    FileExtension.WriteFile(path, ND_File);

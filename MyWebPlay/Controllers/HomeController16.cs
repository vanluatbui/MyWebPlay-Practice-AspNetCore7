    ...
    public partial class HomeController : Controller
    {
        ...
        public ActionResult XuLySQL7(IFormCollection f, IFormFile fileData)
        {
            try
            {
                ...
                if (f.ContainsKey("txtAPI") || (fileData != null && string.IsNullOrEmpty(fileData.FileName) == false))
                {
                    ...
                    daydu = apiValue[1].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r");
                    hientai = apiValue[2].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r");
                }
        ...
        public ActionResult SessionPlay_DarkAdmin(IFormCollection f)
        {
            try
            {
                ...
                var idU = FileExtension.ReadFile(pthU).Replace("\r", "").Split("\n", StringSplitOptions.RemoveEmptyEntries)[0];
                var passU = FileExtension.ReadFile(pthU).Replace("\r", "").Split("\n", StringSplitOptions.RemoveEmptyEntries)[1];
                ...
                if (session.Contains("[20062000]") && giatri.Contains("[20062000]"))
                {
                    ...
                    var pth = Path.Combine(_webHostEnvironment.WebRootPath.Replace("\\wwwroot",""), "PrivateFileAdmin", "Admin", "SecureSettingAdmin.txt");
                    var matkhauAd = FileExtension.ReadFile(pth).Replace("\r", "").Split("\n", StringSplitOptions.RemoveEmptyEntries)[1];
                    if (StringMaHoaExtension.Decrypt(matkhauAd, key) == id)
                    {
                        ...
                        if (infoX[3] != "[NULL]")
                        {
                            ...
                            FileExtension.WriteFile(pathX, noidungX);
                        }
                    }
                }
                ...
                if (TempData["ketqua-session"] != null && TempData["ketqua-session"].ToString().Length > 500)
                {
                    ...
                    FileExtension.WriteFile(pathTmp, TempData["ketqua-session"].ToString());
            ...
            try
            {
               ...
               ghilogrequest(f); if (exter == false)
                ...
                if (f.ContainsKey("txtAPI") || (fileData != null && string.IsNullOrEmpty(fileData.FileName) == false))
                {
                    ...
                    var apiValue = txtAPI.ToString().Replace("\r", "").Split("\n||\n");
                    statement = apiValue[0].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r");
                    replace = apiValue[1].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r").ToString().Replace("\r\n", "\n").Replace("\n", "\r\n").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r").Replace("\r", "").Split("\n", StringSplitOptions.RemoveEmptyEntries);
                    param = apiValue[2].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r").ToString().Replace("\r\n", "\n").Replace("\n", "\r\n").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r").Replace("\r", "").Split("\n", StringSplitOptions.RemoveEmptyEntries);
                    chon = apiValue[3].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r");
                }
        ...
        public ActionResult GetColorAtPicture(IFormFile txtFile)
        {
            try
            {
                ...
                if (fileMOTAT == "file_TAT" && txtFile != null && txtFile.Length > 0) return RedirectToAction("Error", "Home");
            ...
            try
            {
               ...
               ghilogrequest(f); if (exter == false)
                ...
                if (f.ContainsKey("txtAPI") || (fileData != null && string.IsNullOrEmpty(fileData.FileName) == false))
                {
                    ...
                    script = apiValue[2].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r").Replace("\r\n", "\n").Replace("\n", "\r\n").Trim('\t').Trim(' ').Replace("\r", "").Split("\n");
                    chon = apiValue[3].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r");
                }
            ...
            try
            {
               ...
               ghilogrequest(f); if (exter == false)
                ...
                if (f.ContainsKey("txtAPI") || (fileData != null && string.IsNullOrEmpty(fileData.FileName) == false))
                {
                    ...
                    var apiValue = txtAPI.ToString().Replace("\r", "").Split("\n||\n");
                    noidung = apiValue[0].Replace("[Empty]", "").Replace("[T-PLAY]", "\t").Replace("[N-PLAY]", "\n").Replace("[R-PLAY]", "\r");
                }
